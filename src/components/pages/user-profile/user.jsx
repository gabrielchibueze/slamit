...
export default function User() {
  ...
  const handleFetchThisUser = () => {
    fetch(`https://slamit-d27722e9cea6.herokuapp.com/auth/${id}`, {
      method: "GET",
      headers: {
  ...
  useEffect(() => {
    ...
    if (state.isAuthenticated && usersFollowers) {
      setCurrentState((prevState) => {
        return {
          ...prevState,
          isFollowing: usersFollowers.find(
            (userId) => userId === state.user._id
          ),
        };
      });
    }
  ...
  const handleFollowUser = useCallback(
    (followOrUnfollow, currentUserId, followedUserId) => {
          ...
          <div className="main-outlet">
            ...
            <div className="user-profile">
              {currentState.user?.status &&
                currentState.user.status.length !== 0 && (
              ...
              <div className="user-profile__top-section">
                <div className="main-user__top-section">
                  <div className="user-personal__section">
                    <div>
                      {currentState.user?.photo ? (
                        <img
                          src={`${currentState.user.photo}`}
                          className="user-profile__photo"
                        />
                      ) : (
                        <img
                          src={"/avataricon.png"}
                          className="user-profile__photo avatar"
                        />
                      )}
                    </div>

                    <div className="user-personal__details">
                      {state.isAuthenticated && id === state.user?._id && (
                        <h2>{currentState.user?.fullname}</h2>
                      )}
                      <p className={id != state.user?._id ? "bolden" : ""}>
                        @{currentState.user?.username.split()[0].toLowerCase()}
                      </p>
                    </div>
                  </div>
                  {currentState.user?._id !== state.user?._id && (
                    <div>
                      {currentState.isFollowing && state.isAuthenticated ? (
                        <p
                          className="follow__unfollow"
                          onClick={() =>
                            handleFollowUser("unfollow", currentState.user?._id)
                          }
                        >
                          Unfollow
                        </p>
                        ...
                        <p
                          className="follow__unfollow"
                          onClick={() =>
                            handleFollowUser("follow", currentState.user?._id)
                          }
                        >
                          Follow +
                        </p>
                ...
                <div className="user-activities">
                  <p>
                    <span>{currentState.user?.followers?.length || 0} </span>
                    Follower
                    {currentState.user?.followers?.length <= 1 ? "" : "s"}
                  </p>
                  <p>
                    <span>{currentState.user?.following?.length || 0} </span>
                    Following
                    {currentState.user?.following.length <= 1 ? "" : "s"}
                  </p>
                  <p>
                    <span>{currentState.user?.posts.length || 0} </span>Slam
                    {currentState.user?.posts.length <= 1 ? "" : "s"}
                  </p>
                  <p>
                    <span>{currentState.user?.likes.length || 0} </span>Like
                    {currentState.user?.likes.length <= 1 ? "" : "s"}
                  </p>
                </div>
              </div>
