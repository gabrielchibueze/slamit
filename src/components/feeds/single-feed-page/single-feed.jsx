  ...
  async function fetchSinglePost() {
    try {
      ...
      const response = await fetch(
        `https://slamit-d27722e9cea6.herokuapp.com/feeds/posts/${postId}`
      );
      ...
      setCurrentState((prevState) => {
        ...
        return {
          ...
          post: [adjustedPost],
          postLoading: false,
          postLikes: adjustedPost.likes.length,
        };
      });
      setState((prevState) => {
    ...
    <div>
      <ErrorBoundary>
        ...
        <div className="single-post-details__page">
          ...
          {currentState.post ? (
            currentState.post.map((postDetail) => {
              return (
                <div key={postDetail._id}>
                  <div className="top-like__delete-section">
                    <div className="like-section">
                      <div className="like-buttons">
                        {currentState.likedPostHistory.find(
                          (post) => post === postDetail._id
                        ) ||
                        (currentState.isLiked &&
                          currentState.postId === postDetail._id) ? (
                          <p
                            className="like"
                            onClick={() =>
                              clickOnLike(postDetail._id, "dislike")
                            }
                          >
                            <FaHeart />
                          </p>
                          ...
                          <p
                            className="dislike"
                            onClick={() => clickOnLike(postDetail._id, "like")}
                          >
                            <FaRegHeart />
                          </p>
                        )}
                    ...
                    {postDetail.creator._id !== state.user?._id && (
                      <div>
                        {(currentState.followings?.length &&
                          currentState.followings.find(
                            (user) => user === postDetail.creator._id
                          )) ||
                        currentState.isFollowing ? (
                          ...
                          <p
                            className="follow__unfollow"
                            onClick={() =>
                              handleFollowUser("follow", postDetail.creator._id)
                            }
                          >
                            Follow +
                          </p>
                    ...
                    {state.isAuthenticated &&
                      state.user?._id.toString() ===
                        postDetail.creator._id.toString() && (
                            ...
                            buttonProperties: [
                              {
                                ...
                                buttonLink: null,
                                buttonFunction: () =>
                                  startEditPostHandler(postDetail._id),
                              },
                              {
                                buttonType: "reset",
                                buttonTitle: "Delete",
                                ...
                                buttonLink: null,
                                buttonFunction: () =>
                                  deletePostRequest(postDetail._id),
                              },
                            ],
                          }}
                        />
                  ...
                  <p className="single-post__creator">
                    Slam created by
                    <span className="posted-by">
                      <Link
                        to={`/user/${postDetail.creator._id}`}
                        style={{
                          color: "rgb(37, 13, 75)",
                      ...
                      >
                        {postDetail.creator.username || "Anonymuous User"}
                      </Link>
                    </span>
                    on
