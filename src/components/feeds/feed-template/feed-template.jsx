...
export default function FeedTemplate({ props }) {
  const { state, setState, likePost, followUser, emitContent, catchError } =
  ...
  useEffect(() => {
    ...
    if (state.isAuthenticated) {
      ...
      setCurrentState((prevState) => {
        return {
          ...prevState,
          likedPostHistory: usersAllLiked,
        };
      });
    }
  }, [state]);
      ...
      <div className="slam-feed-section">
        <div className="feed-template__top-section">
          <div className="post-links">
            <p className="author-date__section">
              Slammed by
              <span className="posted-by">
                <Link
                  to={props.creatorId ? `/user/${props.creatorId}` : ""}
                  style={{
                    color: "rgb(37, 13, 75)",
                    textDecoration: "underline",
                  }}
                >
                  {props?.creator || "Anonymuous User"}
                </Link>
              </span>
              <span className="createdAt-time">
                on {props?.createdAt || "23/04/2024"}
              </span>
            </p>
          </div>
          {state.user?._id !== props.creatorId && (
            <div>
              {currentState.followings.find(
                (user) => user === props.creatorId
              ) ||
              (currentState.isFollowing &&
                currentState.creatorId === props.creatorId) ? (
                <p
                  className="follow__unfollow follow_unfollow-extras"
                  onClick={() => handleFollowUser("unfollow", props.creatorId)}
                >
                  Unfollow
                </p>
              ) : (
                <p
                  className="follow__unfollow follow_unfollow-extras"
                  onClick={() => handleFollowUser("follow", props.creatorId)}
                >
                  Follow
                </p>
              )}
        ...
        <div className="content-section">
          <Link className="post-links" to={`/feeds/${props.postId}`}>
          ...
          <div className="post-section">
            ...
            {props.content ? (
              <p>{emitContent(40, props.content)}</p>
            ) : (
              <p>
                Message Snippt goes here. This is the message content. Click to
            ...
            <div className="buttons__feed-templates">
              <div className="like-section">
                <div className="like-buttons">
                  {currentState.likedPostHistory.find(
                    (post) => post === props.postId
                  ) ||
                  (currentState.isLiked &&
                    currentState.postId === props.postId) ? (
                    <p
                      className="like"
                      onClick={() => clickOnLike(props.postId, "dislike")}
                    >
                      <FaHeart />
                    </p>
                  ) : (
                    <p
                      className="dislike"
                      onClick={() => clickOnLike(props.postId, "like")}
                    >
                      <FaRegHeart />
                    </p>
                  )}
                ...
                <p>
                  {currentState.postLikes} Like
                  <span>{currentState.postLikes <= 1 ? "" : "s"}</span>
                </p>
              </div>
                ...
                props={{
                  buttonProperties: [
                    ...
                    },
                    {
                      ...
                      design: "raised",
                      buttonLink: null,
                      buttonFunction: props.startEditPostHandler,
                    },
                    {
                      buttonType: "reset",
                      buttonTitle:
                      ...
                      design: "danger",
                      buttonLink: null,
                      buttonFunction: props.deletePostHandler,
                    },
                  ],
                }}
              />
