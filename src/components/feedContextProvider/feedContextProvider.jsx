...
export default function FeedContextProvider({ children }) {
  ...
  const editPost = (post) => {
    setState((prevState) => {
      let updatedPost = prevState.posts ? [...prevState.posts] : [];
      let updatedPostIndex = prevState.posts.findIndex(
        (pst) => pst._id === post._id
      );
  ...
  const loadPosts = (direction) => {
    ...
    fetch(
      `https://slamit-d27722e9cea6.herokuapp.com/feeds/posts?limit=${state.itemsPerPage}&page=${state.postPage}`
    )
      .then((res) => {
        if (!res.ok) {
  ...
  const handleFetchUser = () => {
    ...
    fetch(
      `https://slampost-8dd6d1d06367.herokuapp.com/auth/${state.user?._id}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const emitContent = (contentLength, content) => {
    ...
    if (state.miniDesktop && content.length > contentLength + 93) {
      ...
      !state.miniDesktop &&
      !state.mobileView &&
      content.length > contentLength + 240
    ) {
  ...
  const likePost = async (postId, likeOrDislike) => {
    ...
    try {
      ...
      const sendLike = await fetch(
        "https://slampost-8dd6d1d06367.herokuapp.com/feeds/likes",
        {
          method: "put",
          body: JSON.stringify({
            postId: postId,
  ...
  const followUser = async (followOrUnfollow, followedUserId) => {
    try {
      ...
      const sendFollowRequest = await fetch(
        "https://slampost-8dd6d1d06367.herokuapp.com/feeds/follow",
        {
          method: "put",
          body: JSON.stringify({
  ...
  const deletePostHandler = (postId) => {
    ...
    fetch(
      `https://slampost-8dd6d1d06367.herokuapp.com/feeds/delete/${postId}`,
      {
        method: "DELETE",
        headers: {
          Authorization: "Bearers " + state.token,
